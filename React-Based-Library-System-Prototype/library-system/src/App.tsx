...
export default function App() {
  ...
  // 1. Process Daily Log (Clean Input.csv)
      ...
      Papa.parse(file, {
        ...
        complete: (results: ParseResult<any[]>) => {
          ...
          dataRows.forEach(row => {
            const dateStr = row[4]; // Date column
            if (dateStr && !logDate) logDate = dateStr;
            const prog = String(row[5] || "").trim().toUpperCase(); // Program column
            if (prog) {
  ...
  const processAndDownload = () => {
    ...
    // Search row 1 for the day column
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row justify-between items-end border-b-2 border-red-900 pb-8">
          ...
          <div className="bg-black/40 p-5 rounded-3xl border border-red-900/50 min-w-[220px] backdrop-blur-md">
            <p className="text-red-500 text-[10px] font-black uppercase tracking-widest flex items-center justify-end gap-2 text-right">
              <Heart size={10} fill="currentColor" /> Developed By
            </p>
            <p className="text-white font-black text-xl uppercase tracking-tight text-right">Group 7</p>
          </div>
        </header>
            ...
            <div className="bg-black/80 rounded-[3rem] border-2 border-slate-900 shadow-2xl min-h-[500px] flex flex-col backdrop-blur-sm overflow-hidden">
              <div className="p-8 border-b-2 border-slate-900 flex justify-between items-center">
                <h3 className="text-white font-black text-xl tracking-tighter uppercase italic flex items-center gap-3">
                  <span className="w-3 h-3 bg-red-600 rounded-full animate-pulse shadow-[0_0_10px_rgba(220,38,38,1)]" /> Live Monitor
                </h3>
              ...
              <div className="p-10 flex-1 overflow-y-auto">
                {Object.keys(tallies).length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-20"><Database size={64} className="text-white" /><p className="font-black tracking-[0.5em] uppercase text-[10px] mt-4 text-white">Awaiting Files...</p></div>
                )}
              </div>
            </div>
          </div>
